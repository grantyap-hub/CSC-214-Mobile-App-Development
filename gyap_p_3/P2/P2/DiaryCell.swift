...
class DiaryCell: UITableViewCell {
    ...
    func update(with item: Item){
        if let title = item.value(forKey: "title") as? String,
            ...
            mileageLabel?.text =  NSLocalizedString("Kills: ", comment: "") + mileage.description
            durationLabel?.text =  NSLocalizedString("Deaths: ", comment: "") + duration.description
            contactLabel?.text =  NSLocalizedString("Assists: ", comment: "") + contact.description
            
//            NSLocalizedString("Mileage:", comment: "") +
//            NSLocalizedString("Duration:", comment: "") +
//            NSLocalizedString("Contacts:", comment: "") +
            if let purposeType = ChampType(rawValue: purpose){
